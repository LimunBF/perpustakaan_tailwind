...
if (isset($_POST['pinjam'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $stmt = mysqli_prepare($con, $sql);
        mysqli_stmt_bind_param($stmt, "s", $nama_peminjam);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        ...
        if (mysqli_num_rows($result) == 0) {
            ...
            $sql = "INSERT INTO peminjaman (username, isbn, jumlah_pinjam, tanggal_pinjam, tanggal_kembali, status) VALUES (?, ?, ?, ?, ?, 'borrowed')";
            $stmt = mysqli_prepare($con, $sql);
            mysqli_stmt_bind_param($stmt, "siiss",  $nama_peminjam,$isbn, 1, $tanggal_pinjam, $tanggal_kembali);
            mysqli_stmt_execute($stmt);
...
if (isset($_POST['searchTerm'])) {
    ...
    $sql = "SELECT JUDUL, ISBN FROM books WHERE JUDUL LIKE ?";
    $stmt = mysqli_prepare($con, $sql);
    mysqli_stmt_bind_param($stmt, "iiiss", $book_id, $user_id, $tanggal_pinjam, $tanggal_kembali);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
